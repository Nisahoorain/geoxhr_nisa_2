...
engine = create_engine('mysql+pymysql://root:@localhost/geoxhr')
#
...
def candidate():
    ...
    unique_subjects = db.session.query(distinct(Emails_data.subject_part2), Emails_data.id).order_by(
        desc(Emails_data.id)).all()
    unique_lines_set = set()
    for subject_tuple in unique_subjects:
        subject_line = subject_tuple[0]
        if " for " and " applied for " in subject_line:
            after_for = subject_line.split(" for ", 1)[1].strip()
            unique_lines_set.add(after_for)

    unique_lines = sorted(list(unique_lines_set))  # Sort the list alphabetically
    print(unique_lines)
    return render_template('candidates.html', alldata=alldata, page=page, total_pages=total_pages,
                           all_countries=all_countries, phone_codes=phone_codes,start_page=start_page, end_page=end_page,unique_lines=unique_lines)
...
def selecteddata():
    ...
    print(unique_lines)
    return render_template('candidates.html', alldata=alldata, page=page, total_pages=total_pages,
                           all_countries=all_countries, phone_codes=phone_codes, start_page=start_page,unique_lines=unique_lines,
                           end_page=end_page)
...
def registereddata():
    ...
    return render_template('candidates.html', registereddata=alldata, page=page, total_pages=total_pages,
                           ...
                           end_page=end_page,unique_lines=unique_lines)
...
@app.route('/search_subj', methods=['POST'])
@role_required(allowed_roles=['user', 'admin'])
def search():
    data = request.json
    keyword = data.get('keywordInput', '').strip().lower()
    current_url = data.get('currentUrl')
    print(current_url, "currentUrl")
    print(f"Searching for keyword: {keyword},")

    matching_emails = []
    common_filter_conditions = (Emails_data.subject_part2.ilike(f"%{keyword}%"))

    if current_url == 'candidate':
        filter_conditions = common_filter_conditions & (Emails_data.action != 'Interested')
    elif current_url == 'selecteddata':
        filter_conditions = common_filter_conditions & (Emails_data.action == 'Interested')
    elif current_url == 'registereddata':
        filter_conditions = common_filter_conditions & (Emails_data.action != 'Interested') & (Emails_data.subject_part1 == 'registered from Geox hr')
    else:
        filter_conditions = common_filter_conditions

    pdf_data = Emails_data.query.filter(
        filter_conditions
    ).order_by(desc(Emails_data.id)).all()
    matching_emails = [
        {
            "id": email.id,
            "email": email.email,
            ...
            "subject_part2": email.subject_part2,
            "formatted_date": email.formatted_date,
            "pdf_content_json": email.subject_part2,  # If subject_part2 is already a JSON string
            "phone_number": email.phone_number,
            "status": email.status,
        }
        for email in pdf_data
...
def search_country():
    ...
    keyword = data.get('keywordInput', '').strip().lower()
    current_url = data.get('currentUrl')
    keyword_from_url = request.args.get('keyword', '')

    page = request.args.get('page', default=1, type=int)
    per_page = 300  # Adjust per_page as needed

    print(f"Searching for keyword: {keyword_from_url}, current URL: {current_url}, Page: {page}")


    matching_emails = []

    country_prefixes = {
        'canada': ['+1', '647', '622', '1', '403', '587', '825', '368', '780', '604', '778', '236', '672', '250', '204','289'],
        'pakistan': ['+92', '03', '92'],
        'india': ['+91', '11', '79', '183', '80', '265', '755', '22', '33', '172', '40', '141', '181', '512', '44', '20', '261'],
        'mexico': ['+52-187', '+52-800','52-187', '52-800','52-187','+52','52'],
        'philippines': ['+63-999', '+63-001', '63-001', '63-999', '+63', '63']
    }

    if current_url in ['candidate', 'selecteddata', 'registereddata']:
        if keyword in country_prefixes:
            filter_conditions = [
                or_(Emails_data.phone_number.startswith(prefix) for prefix in country_prefixes[keyword])
            ]
            if current_url == 'registereddata':
                filter_conditions.append(Emails_data.subject_part1 == 'registered from Geox hr')
            pdf_data = Emails_data.query.filter(
                or_(*filter_conditions),
                Emails_data.action != 'Interested' if current_url != 'selecteddata' else Emails_data.action == 'Interested'
            ).order_by(desc(Emails_data.id)).all()
    ...
    total_records = len(matching_emails)
    total_pages = ceil(total_records / per_page)
    start_index = (page - 1) * per_page
    end_index = start_index + per_page

    paginated_results = matching_emails[start_index:end_index]
...
def postjob():
    if request.method == 'POST':
        ...
        # Assign numerical values based on selected companies
        if "Geox hr" in selected_companies and "Hands hr" in selected_companies:
            company_code = "12"
        ...
        if id is not None:
            ...
            entry = Jobs(user_id=user_id, name=name, title=title, image=relative_file_path, company=company_code, location=location,
                            job_type=JobType, duration= duration, onsite= onsite, salary_type=salarytypes, salary=Salary,
                            job_date=date, job_status=job_status, description=description, responsibility=responsibility, eligibility=eligibility,experience=experience,
                            notes=notes)
        db.session.add(entry)
        db.session.add(entry)
        db.session.commit()
...
def resettarget():
    if request.method == 'POST':
        ...
        for i in range(len(user_ids)):
            ...
            # Use update method
            weekstart_date = datetime.strptime(weekstart[i], '%Y-%m-%d').date()
            weekend_date = datetime.strptime(weekend[i], '%Y-%m-%d').date()
...
def savetarget():
    if request.method == 'POST':
        ...
        green = 2
        yellow = 1
        print("Lengths:", len(user_ids), len(names), len(news), len(notsigneds), len(reopens), len(resumes), len(interviews), len(placements), len(helpings), len(targets), len(scores))
        print("User IDs:", user_ids)
        print("Names:", names)
        ...
        user_emails = Users.query.filter(Users.id.in_(user_ids),Users.designation !='admin').with_entities(Users.email).all()
